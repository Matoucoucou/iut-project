...
module.exports = [{
    ...
    options: {
        ...
        validate: {
            payload: Joi.object({
                email: Joi.string().required().example('john.doe@example.com').email().description('Email of the user'),
                password: Joi.string().required().example('password').min(8).description('Password of the user')
            })
        }
    },
    handler: async (request, h) => {
